...
import { GLIDE_CALIBRATION } from "../game/glide.ts";
...
Coordinates: x is right (-${L.lane.x}..${L.lane.x}), y is up (${L.lane.yMin}..${L.lane.yMax}), z is forward (0..${L.lane.zMax}). The camera starts at the start entity, looking along +z at yaw 0 and pitch 0. The route must be readable from a first-person camera: z must strictly increase start < checkpoint 1 < checkpoint 2 < checkpoint 3 < goal, each consecutive segment must advance z by ${L.spacing.min}–${L.spacing.max}, move x by at most ${L.segment.maxLateral} and y by at most ${L.segment.maxVertical}. Exactly five entities: one "start" (radius ${L.radius.start.min}–${L.radius.start.max}), three "checkpoint" (radius ${L.radius.checkpoint.min}–${L.radius.checkpoint.max}), one "goal" (radius ${L.radius.goal.min}–${L.radius.goal.max}). Ids are lowercase slugs and unique; rules.requiredCheckpointIds lists the three checkpoint ids in route order and rules.goalEntityId names the goal. durationSeconds is ${L.duration.min}–${L.duration.max}; use 70 seconds by default. Total forward distance from start to goal must be ${L.course.minDistance}–${L.course.maxDistance} units, ideally about 1500. Base speed is ${GLIDE_CALIBRATION.baseSpeed} units/second and boost speed is ${GLIDE_CALIBRATION.boostSpeed}; leave slack for steering and exploration. Create broad alternating left/right arcs, spanning at least ${L.course.minLateralSpan} units horizontally with at least ${L.course.minDirectionChanges} horizontal direction changes between consecutive segments. Vary altitude gently too. The first hoop can be nearer than the later hoops; keep the route visible, spacious and enjoyable to steer. respawnBehindDistance is ${L.respawnBehind.min}–${L.respawnBehind.max}. Mechanic multipliers lift, drag, turnRate, boost are ${L.mechanic.min}–${L.mechanic.max}; 1 is the calibrated default and is almost always right.

World: basePrompt (max ${L.basePrompt} characters) starts with the image's actual medium and material treatment, then describes its main forms and how the place continues ahead. Preserve palette, dark/light balance, edge marks, texture density and depicted lighting. Describe what surfaces LOOK like, not merely their real-world names: black painted water should be described as broad matte near-black ink fills with sparse brush marks, not reflective ocean waves. A drawing remains a drawing, a collage retains cut-paper shapes, and a photograph retains its photographic surfaces. Avoid introducing cinematic realism, glossy reflections, ripples or extra detail unless clearly present in the image. Use positive concrete prose in the output. Never mention cameras, controls, players, games, UI, text, or instructions in basePrompt. landmarks lists ${L.landmarks.min}–${L.landmarks.max} concrete visible things from the image (max ${L.landmark} characters each) that the rings can be placed near; entity labels (max ${L.label} characters) should reference them. perspective is always "first_person".
